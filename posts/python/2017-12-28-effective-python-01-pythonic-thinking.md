...
# 2017-12-28-effective-python-01-pythonic-thinking
...
* 注意内置的 zip 有两个问题：
  ...
  * 如果输入的迭代器长度不同，那么 zip 会表现出比较奇怪的行为。若不能确定zip所封装的列表是否等长，则可考虑改用 itertools 内置模块中的 zip\_longest 函数\(在python2中叫做 izip\_longest\)
* 要点
  ...
  * itertools 内置模块中的zip\_lognest 函数可以平行地遍历多个迭代器，而不用在乎它们的长度是否相等
