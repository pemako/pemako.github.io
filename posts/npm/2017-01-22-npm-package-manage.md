...
`npm install <package_name>` 这个命令会在当前的目录下创建 `node_modules` 文件夹。如果需要全局安装包的话加上 `-g` 参数
...
删除 node\_modules 中的模块可以直接是用 `npm uninstall <package>`
...
1. `npm adduser`
2. `npm login`
3. `npm publish` 发布完成可以在`https://www.nmpjs.com` 中查看
4. 使用刚发布的包 `npm install npm-demo-pkg424`
5. `echo "var demo = require("npm-demo-pkg424"); demo.printMsg();"> test.js && node test.js`

## 注意事项

### Semver for publishers

如果你的版本将要发布供其他人使用的话，版本应该从 `1.0.0` 开始。 当你的包有需要升级时，按照下面的原则修改版本号
