...
#### 1.3.1【必须】命令执行检查
- 程序使用exec.Command函数执行系统命令时，如果涉及的命令由外部传入，必须要对传入的参数进行判断，限制其只能执行特定命令。对于采用bash、sh执行的，还要过滤`&、|、 ；`等命令拼接字符防止代码注入执行。

```go
// bad
func foo() {
    cmdName := "echo hello && echo 'command inject'"
...
- 禁止在源码中硬编码`IP`信息
- 禁止在源码中硬编码员工敏感信息，包括但不限于：`员工ID`、`手机号`、`微信/QQ号`等
...
- tRPC-go框架应使用`validation`拦截器，在probuf文件添加校验规则，所有String类型字段必须启用校验
...
//server开启validation拦截器校验输入参数
server:
 ...
 filter:
  ...
  - validation 
//client开启validation拦截器校验响应参数
client:
 ...
 filter:
  ...
...
- 如请求资源域名归属固定的范围，如只允许`a.qq.com`和`b.qq.com`，应做白名单限制。如不适用白名单，则推荐的校验逻辑步骤是：
  ...
  - 第 6 步、如有跳转，跳转后执行1，否则对URL发起请求
  
- 推荐使用应用漏洞扫描O'Team开发的安全API - [scurl安全函数](https://git.code.oa.com/sec-api/go/tree/master/scurl)

- 若不想对cos地址进行过滤，有将cos地址加入白名单的需求，应将cos资源限制到具体的appid。比如`xxx-1234567890.cos.ap-guangzhou.myqcloud.com`对应的的appid是1234567890，此时可以校验host是否以`-1234567890.cos.ap-guangzhou.myqcloud.com`结尾来判断是否是白名单内的cos地址

```go
// bad：未经校验直接使用http.Do请求外部传入的URL资源地址
import (
	"net/http"
...
// good：使用安全API提供的NewSafeClient来请求不可信资源
import (
	"fmt"
...
- tRPC-Go框架应使用[bkn](http://km.oa.com/articles/view/444295)插件。在probuf文件、前端文件添加csrf防御的相应逻辑。

```go
import (
   _ "git.code.oa.com/trpc-go/trpc-filter/bkn"
)
...
- 没有独立账号体系的外网服务使用`QQ`或`微信`登录，内网服务使用`TOF`登录，其他使用账号密码登录的服务需要增加验证码等二次验证

- tRPC-Go框架可使用[ptlogin](https://git.code.oa.com/trpc-go/trpc-filter/tree/master/ptlogin)、[wxlogin](https://git.code.oa.com/trpc-go/trpc-filter/tree/master/wxlogin)、[qqconnect](https://git.code.oa.com/trpc-go/trpc-filter/tree/master/qqconnect)拦截器鉴权

  ```go
  import (
     _ "git.code.oa.com/trpc-go/trpc-filter/ptlogin"
     _ "git.code.oa.com/trpc-go/trpc-filter/wxlogin"
  )
...
#### 2.1.1【推荐】依赖库来源安全
使用的依赖库须经公司供应链恶意包检测系统`horus`安全检测，或只从腾讯软件源`https://mirrors.tencent.com/go/`下载。